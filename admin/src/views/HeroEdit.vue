      ...
      <el-tabs type="border-card" value="basic">
        <el-tab-pane label="基本信息" name="basic">
          ...
          <el-form-item label="难度">
            <el-rate style="margin-top: .6rem" :max="9" show-score v-model="model.scores.difficlt"></el-rate>
          </el-form-item>
        ...
        <el-tab-pane label="技能" name="skills">
          ...
          <el-row type="flex" style="flex-wrap: wrap">
            <el-col :md="12" v-for="(item,idx) in model.skills" :key="idx">
              ...
              <el-form-item>
                <el-button size="small" type="danger" @click="model.skills.splice(idx,1)">删除</el-button>
              </el-form-item>
            </el-col>
          </el-row>
        </el-tab-pane>
...
export default {
  ...
  methods: {
    ...
    async fetchCategories() {
      ...
      let heroes = [];
      res.data.forEach(it => {
        if (it.parent && it.parent.name == "英雄分类") {
          heroes.push(it);
        }
      });
      this.categories = heroes;
    },
    async fetchItems() {
  ...
  created() {
    ...
    this.fetchHeroes();
    this.id && this.fetch(); // 有this.id才执行
  }
};
</script>